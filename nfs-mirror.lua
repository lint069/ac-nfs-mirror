...
---@param cursorType? ui.MouseCursor @Changes the mouse cursor to
local function tooltip(text, cursorType)
    if ui.itemHovered() then
        if cursorType then ui.setMouseCursor(cursorType) end
        ui.tooltip(vec2(8, 4), function() ui.text(text) end)
    end
end
...
local manifest = ac.INIConfig.load(ac.getFolder(ac.FolderID.ACAppsLua) .. '/nfs_mirror/manifest.ini', ac.INIFormat.Extended)
...
function script.settings()
    ...
    local firstItemOffset = 5

    ui.tabBar('settings', function()
        ui.tabItem('App', function()
            ui.offsetCursorY(firstItemOffset)
            ...
            if not isOnlineRace then
                ...
                ui.textDisabled('More options are only avalible for an online race.')
                return
            end
        ...
        ui.tabItem('About', function()
            ...
            local issueUrl = 'https://github.com/lint069/ac-nfs-mirror/issues/new'

            if ui.button('Report Issue', ui.ButtonFlags.None) then os.openURL(issueUrl, false) end
            tooltip('Requires you to be logged into your github account.\nAlternatively you can message me on discord @wallpaperengineman', ui.MouseCursor.Hand)
